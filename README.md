...
```r
covid_clean <- covid %>%
  select(date, location,  population, new_cases) %>% #choose parameters useful for time series. data of course, but also country and population
  filter(!is.na(location), location != "World",location != "Asia",location != "South America",location != "Europe",location != "North America") %>%   # remove regions so we only have coutnries

#above, we extract only country related data with stats.
#Below, we now try to check the top offenders in covid cases. We can find the max cases and deaths, then use the arrange function in dplyr to arrange from the top. Just #have the first 5 entries.
...
covid_summary <- covid_clean %>%
  ...
  head(10)
#now we plot cases in united states across the years.  We filter location to US specifically. before setting the plot between new_cases and date. 
covid_clean %>%
  filter(location == "United States") %>%
  ggplot(aes(x = date, y = new_cases)) +
  geom_line(color = "steelblue") +
  labs(
    title = "Daily New COVID-19 Cases in the USA",
    x = "Date",
    y = "New Cases"
  ) +
  theme_minimal()

top_countries <- c("United States", "India", "Brazil", "United Kingdom", "Germany")# put in the top 5 we found earlier
covid_clean %>%
  filter(location %in% top_countries) %>% #check if an entry's location is one of these 5 countries. 
  ggplot(aes(x = date, y = new_cases, color = location)) +
  geom_line() +
  labs(title = "Daily New Cases: Top Countries", x = "Date", y = "New Cases") +
  theme_minimal()
```
![Top 5 are visualised here](assets/images/top5.png) 
We can tell that the United states outstripped everybody else by a clear margin, likely owing to Trump's lack of regard for the pandemic. 
![US cases](assets/images/cases.png)  From this graphic we can tell there was a spike in cases in 2020 -2021, which is when the pandemic was at its worse. Then it slipped down again, before rising back up in 2o23, perhaps because people were no longer practicing good hygiene. 
...
```
This Shiny application provides an interactive visualization of daily COVID-19 cases by country. It allows the user to select a specific country from a dropdown menu, after which the application generates a dynamic Plotly chart that displays the trend of new COVID-19 cases over time for the selected location. The user interface is defined using the fluidPage() function, which creates a responsive layout that adjusts to different screen sizes. The first element, titlePanel("COVID-19 Cases by Country"), sets the title that appears at the top of the page. The structure of the interface is organized using sidebarLayout(), which divides the page into two sections: a sidebar panel on the left and a main panel on the right.

Within the sidebar panel, the selectInput() function creates a dropdown menu labeled “Select country.” The choices argument automatically fills this menu with a sorted list of all unique country names from the covid_clean$location column, allowing the user to select any available country in the dataset. The main panel contains a plotlyOutput("covidPlot") element, which acts as a placeholder for the interactive chart that will be generated by the server logic.

The server logic is defined in the server function, which takes two standard Shiny arguments: input and output. The input object stores the user’s selections and interactions—in this case, the chosen country—while the output object holds the results that are dynamically rendered in the user interface. Inside the server function, the output$covidPlot object is created with renderPlotly(), indicating that this output will be an interactive Plotly visualization. The dataset covid_clean is filtered using filter(location == input$country) so that only the data for the selected country is displayed.

The filtered dataset is then passed to plot_ly(), which constructs an interactive scatter plot. The x variable corresponds to the date column, and the y variable corresponds to new_cases, showing how daily COVID-19 case counts change over time. The type argument is set to 'scatter', and mode = 'lines+markers' ensures that both a continuous line and distinct data points appear on the chart. The name = "Daily New Cases" argument gives the plotted data a label that will appear in the chart’s legend.

To refine the chart’s appearance, the layout() function is used. It adds a descriptive title that dynamically includes the name of the selected country and defines labels for both axes—“Date” for the x-axis and “New Cases” for the y-axis. This layout ensures that the visualization is informative and easy to interpret.

Finally, the shinyApp(ui = ui, server = server) command brings the user interface and server logic together to create a fully functional application. When this command is run, the app launches in the user’s web browser, allowing them to interactively explore COVID-19 case trends for different countries in real time.
![Shiny](assets/images/dashboard.png) Above is demonstrated the dashboard graphic that allow one to select data by country in the bar. We can see in general poorer countries tended to fare worse, ostensibly because of bad healthcare amenities. 
